...
# `$0` checks a docker-based RepoGate deployment in a container.
...
test_proj_host_pass="$5"
test_proj_host="$6"
test_proj_user="$7"
test_proj_name="$8"

https_port=$(docker port "$cont_name" 8080 | sed 's/.*://')
gitd_port=$(docker port "$cont_name" 9418 | sed 's/.*://')
wall_port=$(docker port "$cont_name" 9000 | sed 's/.*://')
