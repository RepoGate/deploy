            ...
            req.on('end', function () {
                ...
                seq(
                    ...
                    [
                        ...
                        seqExecInDir(
                            'bash /home/repogate/deploy_rg/scripts/clone.sh'+
                            ' '+form.host+
                            ' '+form.user+
                            ' '+form.project+
                            ' '+'MASTER_PASSWORD'+
    ...
    '/projects/delete': readProject(
        ...
        function (req, resp, projName) {
            seq(
                null,
                [
                    seqExec('rm -rf /home/repogate/deploy_rg/repos/'+projName),
                    seqExec('rm -rf /var/tmp/repo_update/'+projName),
                    seqExec(
                        'bash /home/repogate/deploy_rg/scripts/secrets/rm.sh'+
                        ' '+process.env.MASTER_PASSWORD+
                        ' '+'/home/repogate/deploy_rg/repos_pass.aes'+
                        ' '+projName
                    ),
...
var secretsDir = '/home/repogate/deploy_rg/secrets';
https.createServer(
    {
        key: fs.readFileSync(secretsDir+'/privkey.pem'),
        cert: fs.readFileSync(secretsDir+'/cert.pem')
    },
    function (req, resp) {
